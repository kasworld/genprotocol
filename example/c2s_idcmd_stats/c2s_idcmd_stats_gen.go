...
type CommandIDStat [c2s_idcmd.CommandID_Count]int
...
func (es *CommandIDStat) String() string {
	...
	for i, v := range es {
		fmt.Fprintf(&buf,
			"%v:%v ",
			c2s_idcmd.CommandID(i), v)
	}
	buf.WriteString("]")
	return buf.String()
}
func (es *CommandIDStat) Inc(e c2s_idcmd.CommandID) {
	es[e] += 1
}
func (es *CommandIDStat) Add(e c2s_idcmd.CommandID, v int) {
	es[e] += v
}
func (es *CommandIDStat) SetIfGt(e c2s_idcmd.CommandID, v int) {
	if es[e] < v {
		es[e] = v
	}
}
func (es *CommandIDStat) Get(e c2s_idcmd.CommandID) int {
	return es[e]
}

// Iter return true if iter stop, return false if iter all
// fn return true to stop iter
func (es CommandIDStat) Iter(fn func(i c2s_idcmd.CommandID, v int) bool) bool {
	for i, v := range es {
		if fn(c2s_idcmd.CommandID(i), v) {
			return true
		}
	}
	return false
...
func Index(i int) string {
	return c2s_idcmd.CommandID(i).String()
}
