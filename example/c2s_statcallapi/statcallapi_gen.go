...
type StatCallAPI [c2s_idcmd.CommandID_Count]StatRow

func New() *StatCallAPI {
	cps := new(StatCallAPI)
	for i := 0; i < c2s_idcmd.CommandID_Count; i++ {
		cps[i].Name = c2s_idcmd.CommandID(i).String()
	}
	return cps
}
func (cps *StatCallAPI) BeforeSendReq(header c2s_packet.Header) (*statObj, error) {
	if int(header.Cmd) >= c2s_idcmd.CommandID_Count {
		return nil, fmt.Errorf("CommandID out of range %v %v",
			header, c2s_idcmd.CommandID_Count)
	}
	return cps[header.Cmd].open(), nil
}
func (cps *StatCallAPI) AfterSendReq(header c2s_packet.Header) error {
	if int(header.Cmd) >= c2s_idcmd.CommandID_Count {
		return fmt.Errorf("CommandID out of range %v %v", header, c2s_idcmd.CommandID_Count)
	}
	n := int(header.BodyLen()) + c2s_packet.HeaderLen
	cps[header.Cmd].addTx(n)
	return nil
}
func (cps *StatCallAPI) AfterRecvRsp(header c2s_packet.Header) error {
	if int(header.Cmd) >= c2s_idcmd.CommandID_Count {
		return fmt.Errorf("CommandID out of range %v %v", header, c2s_idcmd.CommandID_Count)
	}
	n := int(header.BodyLen()) + c2s_packet.HeaderLen
