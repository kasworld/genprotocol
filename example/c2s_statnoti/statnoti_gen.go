...
type StatNotification [c2s_idnoti.NotiID_Count]StatRow

func New() *StatNotification {
	ns := new(StatNotification)
	for i := 0; i < c2s_idnoti.NotiID_Count; i++ {
		ns[i].Name = c2s_idnoti.NotiID(i).String()
	}
	return ns
}
func (ns *StatNotification) Add(hd c2s_packet.Header) {
	if int(hd.Cmd) >= c2s_idnoti.NotiID_Count {
		return
	}
	ns[hd.Cmd].add(hd)
}
...
func (ps *StatRow) add(hd c2s_packet.Header) {
	ps.mutex.Lock()
	ps.Count++
	n := int(hd.BodyLen()) + c2s_packet.HeaderLen
	ps.TotalByte += n
	if n > ps.MaxByte {
		ps.MaxByte = n
	}
